...
from map_setup import build_grid, OBSTACLE_CENTER, OBSTACLE_RADIUS, GRID_SIZE, START, GOAL
...
fig_t, axes = plt.subplots(1, 2, figsize=(14, 6))
for ax, xs, ys, ts, label, cmap in [
    (axes[0], xf, yf, tf, "Min-Time", "plasma"),
    (axes[1], xe, ye, te, "Min-Energy", "viridis"),
]:
    background_ax(ax)
    sc = ax.scatter(xs, ys, c=ts, cmap=cmap, s=5)
    plt.colorbar(sc, ax=ax, label="Time (s)")
    ax.plot(*START, "go", markersize=8)
    ax.plot(*GOAL, "r*", markersize=10)
    ax.set_title(f"{label} Trajectory")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
fig_t.tight_layout()
fig_t.savefig("results/trajectory_comparison.png", dpi=150)
plt.close(fig_t)
...
print(f"  Min-energy -> time: {t_energy:.1f} s  | energy: {e_energy:.1f}")
print(f"  Time saved  : {t_energy - t_fast:.1f} s ({(t_energy - t_fast)/t_energy*100:.1f}% faster)")
